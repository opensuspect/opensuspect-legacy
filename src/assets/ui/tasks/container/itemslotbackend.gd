...
func set_in_hand(ui_data_pass, index:int) -> void:#This the crucial function
	...
	#By only this the  item from the itemslot get transferred to hand
...
puppetsync func set_path() -> void:#Give the item its location so it can remove itself and add to other location
	var item = frontend.get_child(1)
	var self_path = get_parent().get_path()
	item.item_location = self_path
	item.item_from_container = true

func on_input_received(ui_data:Dictionary, index:int) -> void:
	rpc_id(1, "set_server")
	set_in_hand(ui_data,index)

remotesync func set_server() -> void:
